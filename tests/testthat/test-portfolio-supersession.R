...
submit_initial_spy_target <- function(exchange, execution, agent_id = "agent") {
  eleven <- supersession_bars("2026-01-05 11:00:00", c("SPY", "TLT"))
...
test_that("a later same-asset target supersedes an unfilled target order", {
  ...
  newer <- sim_portfolio_target_submit(
    exchange, "agent", eleven, c(SPY = 1), execution,
    allowed_symbols = c("SPY", "TLT")
  )
  ...
  next_spy <- supersession_bars("2026-01-06 09:00:00", c("SPY", "TLT"))
  ...
  expect_true(all(market$fills$timestamp > eleven$timestamp[1L]))
})
...
test_that("partial market boundaries fill eligible legs but reject multi-asset updates", {
  exchange <- make_supersession_exchange()
  execution <- sim_portfolio_execution(lev = 1)
  ...
  expect_equal(old[symbol == "SPY", status], "accepted")
  expect_equal(exchange$portfolio_rebalances[rebalance_id == "RB000001", status], "accepted")
  expect_equal(nrow(exchange$portfolio_fills[rebalance_id == "RB000001" & symbol == "TLT"]), 1L)
  expect_equal(sim_portfolio_execution_quality(exchange, summary = TRUE)[rebalance_id == "RB000001", execution_quality], "pending")
})

test_that("non-overlapping pending symbols remain eligible and save/load preserves lineage", {
  exchange <- make_supersession_exchange()
  execution <- sim_portfolio_execution(lev = 1)
  boundary <- supersession_bars("2026-03-05 11:00:00", c("SPY", "TLT"))
  sim_portfolio_market_step(exchange, boundary, execution)
  sim_portfolio_target_submit(
    exchange, "agent", boundary, c(SPY = 0.5, TLT = 0.3), execution,
    allowed_symbols = c("SPY", "TLT")
  )
  newer <- sim_portfolio_target_submit(
    exchange, "agent", boundary, c(TLT = 0.5), execution,
    allowed_symbols = c("SPY", "TLT")
  )
  old <- exchange$agent_orders[rebalance_id == "RB000001"]
  expect_equal(old[symbol == "SPY", status], "accepted")
  expect_equal(old[symbol == "TLT", status], "superseded")
  expect_equal(exchange$agent_orders[rebalance_id == newer$rebalance_id, supersedes_rebalance_id], "RB000001")
...
test_that("sequential and batch submission preserve identical supersession outcomes", {
  ...
  boundary <- supersession_bars("2026-04-05 11:00:00", c("SPY", "TLT"))
  sim_portfolio_market_step(sequential, boundary, execution)
  sim_portfolio_market_step(batched, boundary, execution)
  sim_portfolio_target_submit(sequential, "agent", boundary, c(SPY = 0.5), execution, allowed_symbols = c("SPY", "TLT"))
  sim_portfolio_target_submit(batched, "agent", boundary, c(SPY = 0.5), execution, allowed_symbols = c("SPY", "TLT"))
  sim_portfolio_target_submit(sequential, "agent", boundary, c(SPY = 1), execution, allowed_symbols = c("SPY", "TLT"))
  sim_portfolio_target_submit_batch(batched, boundary, list(agent = list(
